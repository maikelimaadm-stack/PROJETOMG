...
try {
  ...
  const touchedOldGate = files.some((f) => /^scripts\/gates\/g423-.*\.mjs$/.test(f) && f !== 'scripts/gates/g423-studio-dev-preview-app-integration-implementation-plan.mjs');
  const touchedOldTest = files.some((f) => /^src\/runtime\/__tests__\/.*\.test\.js$/.test(f) && f !== 'src/runtime/__tests__/studio-dev-preview-app-integration-implementation-plan.test.js');
  ...
  noOldEditDetail = noOldEdit ? 'App.jsx/vite/index.html + productionUiGuard + governance guard + prior gates/tests untouched by this slice' : `touched: ${[touchedGuard ? 'guard' : '', touchedOldGate ? 'old-gate' : '', touchedOldTest ? 'old-test' : '', touchedApp ? 'App.jsx' : '', touchedVite ? 'vite/index.html' : ''].filter(Boolean).join(',')}`;
