...
gate('G423-BR — source handoff contract (strict draft id, no single-draft fallback, fail-closed)', part('createSourceHandoffContract', undefined, (x) => x.kind === 'bridge-source-handoff-contract' && x.strictDraftIdentityRequired === true && x.singleDraftFallbackAllowed === false && x.missingDraftIdFailsClosed === true && x.unknownDraftIdFailsClosed === true && x.requiredFields.length === 11 && x.sourceConsumedReadOnly === true));
...
gate('G423-BR — field mapping (11 lossless, all critical mapped, no unknown transform/default/lossy)', part('createBridgeFieldMappingContract', undefined, (x) => x.kind === 'bridge-field-mapping-contract' && x.mappingCount === 11 && x.everyCriticalMapped === true && x.anyUnknownTransform === false && x.anyCriticalDefault === false && x.anyLossyCritical === false && x.unknownTransformFailsClosed === true && x.lossyCriticalForbidden === true));
...
gate('G423-BR — critical source fields 11', Array.isArray(m?.CRITICAL_SOURCE_FIELDS) && m.CRITICAL_SOURCE_FIELDS.length === 11);
gate('G423-BR — field mappings 11', Array.isArray(m?.BRIDGE_FIELD_MAPPINGS) && m.BRIDGE_FIELD_MAPPINGS.length === 11);
