...
gate('G423-BP — field mapping plan (11, all critical mapped, no unknown transform/default/lossy, not implemented)', part('createFieldMappingExecutionPlan', undefined, (x) => x.mappingCount === 11 && x.everyCriticalMapped === true && x.targetCriticalFieldsAllCovered === true && x.anyUnknownTransform === false && x.anyCriticalDefault === false && x.anyLossyCritical === false && x.mappingExecutorImplemented === false && x.anyImplemented === false && x.mappings.every((mm) => mm.executionStatus === 'planned' && mm.implemented === false)));
...
gate('G423-BP — critical source fields 11', Array.isArray(m?.CRITICAL_SOURCE_FIELDS) && m.CRITICAL_SOURCE_FIELDS.length === 11);
gate('G423-BP — field mappings 11', Array.isArray(m?.BRIDGE_FIELD_MAPPINGS) && m.BRIDGE_FIELD_MAPPINGS.length === 11);
...
try {
  ...
  const touchedUpstream = files.some((f) => /^src\/studio\/blueprint-engine\/(authoring-runtime-to-preview-bridge-contract|module-blueprint-authoring-runtime|module-blueprint-authoring-implementation-plan|module-blueprint-authoring-foundation-contract|module-preview-sandbox|dev-preview-app-integration)\//.test(f));
