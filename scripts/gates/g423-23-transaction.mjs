...
try {
  const source = fs.readFileSync(transactionEnginePath, 'utf8');
  const hasFile = exists(path.join(RUNTIME, 'infra/observability/observabilityEngine.js'));
  const hasReference = /observabilityEngine|ObservabilityEngine/.test(source);
  noObservabilityEngine = !hasFile && !hasReference;
  obsDetail = hasFile ? 'infra/observability/observabilityEngine.js exists' : hasReference ? 'ObservabilityEngine reference found' : 'clean';
} catch (err) {
  obsDetail = err instanceof Error ? err.message : String(err);
}
gate('G423-23 — no Observability Engine created', noObservabilityEngine, obsDetail);
