...
try {
  const source = fs.readFileSync(pluginEnginePath, 'utf8');
  const hasDir = exists(path.join(RUNTIME, 'core/connector'));
  const hasReference = /connectorEngine|ConnectorEngine/.test(source);
  noConnectorEngine = !hasDir && !hasReference;
  connectorDetail = hasDir ? 'core/connector directory exists' : hasReference ? 'ConnectorEngine reference found' : 'clean';
} catch (err) {
  connectorDetail = err instanceof Error ? err.message : String(err);
}
gate('G423-18 — no Connector Engine created', noConnectorEngine, connectorDetail);
