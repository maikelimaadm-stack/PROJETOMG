...
test('481. no backend/prisma in diff', () => { const files = changed(); if (files === null) return; const exempt = createResolvedActiveStudioSlicePathAuthorizer(changed()); assert.ok(!files.filter((x) => !exempt.isAuthorized(x)).some((f) => /^backend\/|schema\.prisma$|^migrations\//.test(f))); });
