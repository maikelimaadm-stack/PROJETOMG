...
test('295. modules/backend/prisma not in diff', () => { const f = changed(); if (f === null) return; const exempt = createResolvedActiveStudioSlicePathAuthorizer(changed()); assert.ok(!f.filter((x) => !exempt.isAuthorized(x)).some((x) => /^src\/modules\/|^backend\/|schema\.prisma$|^migrations\//.test(x))); });
