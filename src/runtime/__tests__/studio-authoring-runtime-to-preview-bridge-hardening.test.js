...
test('277. no guards in diff', () => { const f = changed(); if (f === null) return; assert.ok(!f.includes('scripts/gates/lib/productionUiGuard.mjs'), 'productionUiGuard is never in scope'); if (f.includes('scripts/gates/lib/studioScopeGovernanceGuard.mjs')) { const a = resolveActiveStudioSlice(f); assert.equal(a.ok, true); assert.ok(a.sliceId.startsWith('studio-scope-governance-'), a.sliceId); } });
