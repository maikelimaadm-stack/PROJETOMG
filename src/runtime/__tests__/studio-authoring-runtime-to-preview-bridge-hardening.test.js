...
test('275. no backend/prisma in diff', () => { const f = changed(); if (f === null) return; const exempt = createResolvedActiveStudioSlicePathAuthorizer(changed()); assert.ok(!f.filter((x) => !exempt.isAuthorized(x)).some((x) => /^backend\/|schema\.prisma$|^migrations\//.test(x))); });
