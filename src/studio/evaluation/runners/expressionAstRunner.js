/**
 * Expression AST runner — internal evaluation primitive (Program 2.3.5)
 * Invoked only via Studio Evaluation Pipeline.
 */
...
export function executeExpressionAst(ast, context, functionCatalog = null) {
  const evalNode = (node) => {
    switch (node.kind) {
      case "Literal":
        return node.value;
      ...
      case "CallExpression": {
        ...
        if (functionCatalog && !functionCatalog.has(node.callee)) throw new Error(`Unknown function: ${node.callee}`);
        return null;
      }
      default:
        return null;
