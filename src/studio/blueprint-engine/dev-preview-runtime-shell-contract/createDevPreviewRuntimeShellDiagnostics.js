...
/**
 * Builds passive DIAGNOSTICS for the dev preview runtime shell contract. Pure, metadata-only.
 ...
 */
export function createDevPreviewRuntimeShellDiagnostics(options = {}) {
  ...
  const core = {
    kind: 'dev-preview-runtime-shell-diagnostics',
    passive: true,
    ok: blockers.length === 0,
    headlessConfirmed: v.headless === true && v.contractOnly === true,
    compatible: c.compatibleWithDevPreviewVisualContract !== false,
    blockers: [...blockers],
    warnings: [...warnings],
    blockerCount: blockers.length,
    warningCount: warnings.length,
    notes: blockers.length === 0
      ? ['dev_preview_runtime_shell_contract_headless_ok']
      : ['dev_preview_runtime_shell_contract_has_blockers'],
    logged: false,
    metadataOnly: true,
  };
  return safeCloneGenericModel({ ...core, diagnosticsDigest: shellDigest(core) });
}

export default createDevPreviewRuntimeShellDiagnostics;
