...
export const PREFERENCES_CACHE_UPDATED_EVENT = "mak-preferences-cache-updated";

/** Evento legado — mantido para compatibilidade com e2e e listeners existentes */
export const LEGACY_PREFERENCES_CACHE_UPDATED_EVENT = "emp-preferences-cache-updated";

/** @deprecated compat empresas — alias do evento MAK */
...
/** @type {{ getOriginTabId?: () => string | null, onPreferenceWrite?: (reason: string) => void, isLocalListagemWrite?: (reason: string) => boolean }} */
let cacheHooks = {};

export const configurePreferencesCacheHooks = (hooks = {}) => {
  cacheHooks = { ...cacheHooks, ...hooks };
};

export const resetPreferencesCacheHooks = () => {
  cacheHooks = {};
};
...
export const resolvePreferenceStorageKey = (legacyOrScopedKey) => {
...
const emitCacheUpdate = (keys, reason = "update") => {
  ...
  const detail = {
    ...
    userId: scope.userId,
    version: "v2",
    originTabId: cacheHooks.getOriginTabId?.() ?? null,
  };
  window.dispatchEvent(new CustomEvent(PREFERENCES_CACHE_UPDATED_EVENT, { detail }));
  window.dispatchEvent(new CustomEvent(LEGACY_PREFERENCES_CACHE_UPDATED_EVENT, { detail }));
};
...
export const resetPreferencesMemoryCache = () => {
...
export const withPreferencesCacheBatch = (callback, reason = "batch") => {
...
export const readPreferencesText = (key, fallback = null) => {
...
export const readPreferencesJson = (key, fallback = null) => {
  const raw = readPreferencesText(key, null);
...
export const isLocalListagemPreferenceWrite = (reason = "") => {
  if (typeof cacheHooks.isLocalListagemWrite === "function") {
    return cacheHooks.isLocalListagemWrite(reason);
  }
...
export const isSameTabPreferencesWrite = (detail = {}) => {
  const originTabId = detail?.originTabId;
  const localTabId = cacheHooks.getOriginTabId?.();
  if (!originTabId || !localTabId) return false;
  return originTabId === localTabId;
};

export const writePreferencesText = (key, value, { reason = "update", emit = true } = {}) => {
  ...
  if (changed) {
    if (isLocalListagemPreferenceWrite(reason)) {
      cacheHooks.onPreferenceWrite?.(reason);
    }
    if (emit) {
...
export const writePreferencesJson = (key, value, options = {}) => {
  let serialized = null;
  try {
  ...
  return writePreferencesText(key, serialized, options);
};

export const removePreferencesKey = (key, { reason = "remove", emit = true } = {}) =>
  writePreferencesText(key, null, { reason, emit });

export const hasScopedPreferenceValue = (legacyKey) =>
  readRawPreferenceText(resolvePreferenceStorageKey(legacyKey), null) != null;

export const subscribePreferencesCache = (listener) => {
  ...
  window.addEventListener(PREFERENCES_CACHE_UPDATED_EVENT, handler);
  window.addEventListener(LEGACY_PREFERENCES_CACHE_UPDATED_EVENT, handler);
  return () => {
    window.removeEventListener(PREFERENCES_CACHE_UPDATED_EVENT, handler);
    window.removeEventListener(LEGACY_PREFERENCES_CACHE_UPDATED_EVENT, handler);
  };
};

export const emitPreferencesCacheUpdate = (keys = [], reason = "update") => {
