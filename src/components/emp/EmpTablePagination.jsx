...
export default function EmpTablePagination({
  ...
  return (
    <div className="emp-table-pagination flex shrink-0 items-center justify-end gap-1 border-t border-sky-100 bg-white px-2 py-1">
      <PaginationBtn onClick={() => onPageChange?.(1)} disabled={safePage <= 1} title="Primeira página" aria-label="Primeira página">
        <ChevronsLeft className="w-3 h-3 shrink-0" />
      </PaginationBtn>
      ...
      {pageItems.map((item, index) =>
        item === "ellipsis" ? (
          ...
          <PaginationBtn
            ...
            className="emp-table-pagination-page min-w-[22px] px-1"
          >
            {item}
          </PaginationBtn>
        )
