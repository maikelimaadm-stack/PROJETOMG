...
function DefaultControl({ field, value, onChange, readOnly }) {
  ...
  if (field.type === "textarea") {
    return <Textarea value={value || ""} onChange={(e) => onChange(field.name, e.target.value)} readOnly={readOnly || field.readOnly} placeholder={field.placeholder} className="text-xs uppercase bg-transparent px-2 py-1 min-h-[56px]" rows={field.rows || 2} />;
  }

  if (["select", "autocomplete", "relation"].includes(field.type)) {
    return <EmpAutocomplete items={field.options || []} value={value || ""} onChange={(nextValue) => onChange(field.name, nextValue || "")} placeholder={field.placeholder || "BUSCAR..."} displayField={field.displayField || "nome"} searchFields={field.searchFields || [field.displayField || "nome"]} disabled={readOnly || field.readOnly} readOnly={readOnly || field.readOnly} className="w-full" inputClassName="border-0 shadow-none focus-visible:ring-0 bg-transparent h-7 text-xs px-2" />;
  }

  if (field.type === "checkbox") {
    return <div className="h-7 flex items-center px-2"><ToggleSwitch checked={!!value} onChange={(checked) => onChange(field.name, checked)} disabled={readOnly || field.readOnly} variant="form" /></div>;
  }
...
function FieldFrame({ field, error, children, className = "" }) {
  return (
    <div data-field={field.dataField || field.name} className={`emp-form-field-row grid grid-cols-[190px_minmax(0,1fr)] items-center gap-3 ${field.wide ? "md:col-span-2" : ""}`}>
      <label className="emp-form-field-label text-right leading-none">
        {field.label}:{field.required && <span className="text-red-500 ml-0.5">*</span>}
      </label>
      <div
        className={`emp-form-field-control relative ${field.wide ? "min-h-7" : "h-7"} ${field.medium ? "w-64 max-w-full" : field.compact ? "w-44 max-w-full" : "w-full"} ${error ? "emp-form-field-control-error" : ""} ${className}`}
      >
...
export default function EmpDynamicFormRenderer({ panels = [], fields = [], layout = {}, hiddenFieldIds = [], lockedFieldIds = [], requiredFieldIds = [], visibilityRules = {}, activePanelId, values = {}, errors = {}, onChange, readOnly = false, context = {}, fieldClassName = "" }) {
  ...
  return (
    <div className="emp-form-fields space-y-2.5">
      {visibleFields.length === 0 ? <div className="ml-[191px] text-xs text-slate-500">Nenhum campo configurado para este painel.</div> : visibleFields.map((field) => {
