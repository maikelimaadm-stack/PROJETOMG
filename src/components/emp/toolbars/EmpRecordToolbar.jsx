...
export default function EmpRecordToolbar({ title, operationLabel, badgeLabel = "EMPRESA", showSaveActions = false, showEditAction = false, showDeleteDuplicateActions = true, showUtilityActions = true, onCancel, onSave, onEditRecord, onSettingsClick, onLayoutConfigClick, onAttachClick, attachDisabled = false, onToggleView, onBack, total = 0, currentIndex = 0, onNew, onFirst, onPrevious, onNext, onLast, onDelete, onDuplicate, filterOpen = false, filterActive = false, onToggleFilter, onClearFilter, searchValue = "", onSearchChange, showSearch = false, addButtonClass = iconButtonClass }) {
  ...
  return (
    <div className="bg-white shadow-none overflow-hidden">
      <div className={`flex items-center gap-0 overflow-x-auto whitespace-nowrap bg-white border-y ${toolbarLineClass} [&>*:first-child]:border-l-[0.5px]`}>
        ...
        <Button type="button" variant="outline" size="icon" onClick={onToggleView} className={iconButtonClass} title="Visualizar tabela"><List className="w-3.5 h-3.5" /></Button>
        <Button type="button" variant="outline" size="icon" onClick={onNew} className={addButtonClass}><Plus className="w-4 h-4" /></Button>
        {onToggleFilter && <Button type="button" variant="outline" size="icon" onClick={onToggleFilter} className={filterOpen || filterActive ? "relative h-7 w-9 rounded-none border-y-0 border-l-0 border-r-[0.5px] border-slate-300 bg-white hover:bg-slate-50 text-black shadow-none" : iconButtonClass} title="Filtros"><Filter className="w-3.5 h-3.5" />{filterActive && <span onClick={(e) => { e.stopPropagation(); onClearFilter?.(); }} className="absolute -top-1 -right-1 h-3.5 w-3.5 rounded-full bg-white text-red-600 border border-red-500 text-[10px] leading-[12px] font-bold">×</span>}</Button>}
        ...
        {showEditAction && <Button type="button" variant="outline" size="icon" onClick={onEditRecord} className={iconButtonClass} title="Editar registro"><Pencil className="w-3.5 h-3.5" /></Button>}
        {showDeleteDuplicateActions && <Button type="button" variant="outline" size="icon" onClick={onDelete} disabled={!canNavigate} className={iconButtonClass} title="Excluir registro"><Trash2 className="w-3.5 h-3.5" /></Button>}
        ...
        {showSaveActions && <><Button type="button" variant="outline" size="icon" onClick={onSave} className={iconButtonClass} title="Salvar alterações"><Check className="w-4 h-4" /></Button><Button type="button" variant="outline" size="icon" onClick={onCancel} className={iconButtonClass} title="Descartar"><X className="w-3.5 h-3.5" /></Button></>}
        <div className="ml-auto flex items-center gap-0 [&>*:first-child]:border-l-[0.5px]">
          ...
          {showUtilityActions && <DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="outline" size="icon" className={iconButtonClass} title="Mais opções"><MoreHorizontal className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48 rounded-none p-1"><DropdownMenuItem onClick={onLayoutConfigClick} disabled={!onLayoutConfigClick} className="h-8 cursor-pointer gap-2 text-xs">Layout do formulário</DropdownMenuItem><DropdownMenuItem onClick={onSettingsClick} disabled={!onSettingsClick} className="h-8 cursor-pointer gap-2 text-xs">Campos personalizados</DropdownMenuItem></DropdownMenuContent></DropdownMenu>}
          <div className="h-7 min-w-16 px-3 border-y-0 border-r-[0.5px] border-l-0 border-slate-300 bg-white flex items-center justify-center text-xs text-black">{total > 0 ? `${currentIndex + 1}/${total}` : total}</div>
        </div>
      </div>
      <div className={`h-8 flex items-center gap-2 bg-white border-b-[0.5px] ${toolbarLineClass} px-2`}><span className="w-[150px] h-5 px-1.5 rounded-none border-[0.5px] border-slate-300 text-black text-xs font-semibold text-center truncate inline-flex items-center justify-center">{titleCase(badgeLabel)}</span><span className="text-xs font-semibold text-black truncate min-w-0 flex-1">{title}</span>{operationLabel && <span className="ml-auto text-[11px] font-bold text-black whitespace-nowrap">{titleCase(operationLabel)}</span>}</div>
    </div>
  );
}