    ...
    <div className="bg-white shadow-none overflow-hidden">
      <div className={`flex items-center gap-0 overflow-x-auto whitespace-nowrap bg-white border-y ${toolbarLineClass} [&>*:first-child]:border-l-[0.5px]`}>
        ...
        <div className="ml-auto flex items-center gap-0 [&>*:first-child]:border-l-[0.5px]">
          ...
          <div className="h-7 min-w-16 px-3 border-y-0 border-r-[0.5px] border-l-0 border-slate-300 bg-white flex items-center justify-center text-xs text-slate-500">{total > 0 ? `${currentIndex + 1}/${total}` : total}</div>
        </div>
      </div>
      <div className={`h-8 flex items-center gap-2 bg-white border-b-[0.5px] ${toolbarLineClass} px-2`}><span className="w-[150px] h-5 px-1.5 rounded-none border-[0.5px] border-slate-300 text-slate-500 text-xs font-semibold text-center truncate inline-flex items-center justify-center">{titleCase(badgeLabel)}</span><span className="text-xs font-semibold text-slate-500 truncate min-w-0 flex-1">{title}</span>{operationLabel && <span className="ml-auto text-[11px] font-bold text-slate-500 whitespace-nowrap">{titleCase(operationLabel)}</span>}</div>
    </div>
  );
}