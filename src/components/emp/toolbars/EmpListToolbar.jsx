    ...
    <div className="bg-white">
      <div className={`flex items-center gap-0 overflow-x-auto whitespace-nowrap border-y ${toolbarLineClass} [&>*:first-child]:border-l-[0.5px]`}>
        ...
        <div className="ml-auto flex items-center gap-0 [&>*:first-child]:border-l-[0.5px]">
          ...
          {showUtilityActions && <DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="outline" size="icon" className={iconButtonClass} title="Configurar colunas, exportar e mais opções"><MoreHorizontal className="w-3.5 h-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48 rounded-none p-1"><DropdownMenuItem onClick={onConfigColumns} disabled={!onConfigColumns} className="h-8 cursor-pointer gap-2 text-xs px-1">Configurar colunas</DropdownMenuItem><div className="flex items-center"><DropdownMenuItem onClick={onExportExcel} disabled={!onExportExcel} className="h-8 flex-1 cursor-pointer gap-2 text-xs px-1">Exportar Excel</DropdownMenuItem><button type="button" onClick={onConfigExportExcel} disabled={!onConfigExportExcel} className="h-8 w-8 flex items-center justify-center text-black hover:bg-slate-100 disabled:opacity-50" title="Configurar Excel"><MoreHorizontal className="w-3.5 h-3.5" /></button></div><div className="flex items-center"><DropdownMenuItem onClick={onExportPdf} disabled={!onExportPdf} className="h-8 flex-1 cursor-pointer gap-2 text-xs px-1">Exportar PDF</DropdownMenuItem><button type="button" onClick={onConfigExportPdf} disabled={!onConfigExportPdf} className="h-8 w-8 flex items-center justify-center text-black hover:bg-slate-100 disabled:opacity-50" title="Configurar PDF"><MoreHorizontal className="w-3.5 h-3.5" /></button></div></DropdownMenuContent></DropdownMenu>}
          <div className="h-7 min-w-16 px-3 border-y-0 border-r-[0.5px] border-l-0 bg-white flex items-center justify-center text-xs text-black border-slate-300">{viewMode === "record" && total > 0 ? `${currentIndex + 1}/${total}` : selectedCount > 0 ? `${selectedCount}/${total}` : total}</div>
        </div>
      </div>
      {viewMode === "record" && <div className={`h-8 flex items-center gap-2 bg-white border-b-[0.5px] ${toolbarLineClass} px-2`}>{recordLabel && <span className="px-1.5 py-0.5 rounded-none border-[0.5px] border-slate-300 bg-white text-black text-xs font-semibold">{titleCase(recordLabel)}</span>}<span className="text-xs font-semibold text-black truncate min-w-0 flex-1">{title}</span><span className="ml-auto text-[11px] font-bold text-black whitespace-nowrap">{titleCase(operationLabel || "Visualização de Registro")}</span></div>}
    </div>
  );
}