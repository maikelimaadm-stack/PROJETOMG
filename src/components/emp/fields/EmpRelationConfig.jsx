...
export default function EmpRelationConfig({ form, updateForm, mode = "select" }) {
  ...
  return <><Field label={title}><EmpAutocomplete items={ENTIDADES_RELACIONAIS.map((item) => ({ ...item, id: item.value }))} value={entity || ""} onChange={(value) => handleEntityChange(value || "none")} placeholder="BUSCAR CADASTRO..." displayField="label" searchFields={["label", "value"]} className="w-full" inputClassName="border-0 shadow-none focus-visible:ring-0 bg-transparent h-[22px] text-xs px-1 uppercase" /></Field><div className="ml-[191px] border border-slate-300 bg-slate-50 px-2 py-1 text-xs text-slate-600">{selectedEntity ? `O sistema exibirá automaticamente: ${displayField}` : "Escolha um cadastro para o sistema configurar sozinho."}</div></>;
}