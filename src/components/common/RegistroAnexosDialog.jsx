        ...
        <div className="space-y-1">
          ...
          <div className="border-[0.5px] border-slate-300 bg-white">
            <div className="h-8 flex items-center gap-2 border-b-[0.5px] border-slate-300 px-2">
              <span className="w-[90px] h-5 px-1.5 rounded-none border-[0.5px] border-slate-300 bg-white text-slate-500 text-xs font-semibold text-center truncate inline-flex items-center justify-center">Anexos</span>
              <span className="text-xs font-semibold text-slate-500 truncate flex-1">{title || "Lote"}</span>
              <Button type="button" onClick={() => onOpenChange(false)} title="Fechar" className="rounded-none border-0 bg-white hover:bg-slate-50 text-slate-500 shadow-none h-7 w-7">
                <X className="w-4 h-4" />
              </Button>
            </div>
            <div className="px-4 md:px-8 py-1 w-full space-y-1">
              <div className="grid items-center gap-1 grid-cols-[210px_minmax(0,1fr)]">
                <label className="text-[12px] text-slate-500 text-right leading-none">
                ...
                <div className="grid grid-cols-[minmax(0,1fr)_28px] h-6 border-[0.5px] border-slate-300 bg-white focus-within:border-[#082e54] transition-colors overflow-hidden">
                  ...
                  <Button type="button" variant="outline" size="icon" onClick={() => inputRef.current?.click()} disabled={uploading || !attachmentName.trim()} className="h-[23px] w-7 rounded-none border-y-0 border-r-0 border-l-[0.5px] border-slate-300 bg-white hover:bg-slate-50 text-slate-500 shadow-none p-0" title="Anexar arquivo">
          ...
          <div>
            ...
            <div className="grid grid-cols-[1fr_1.4fr_28px] bg-white border-b-[0.5px] border-slate-300 text-[11px] font-semibold text-slate-500">
            ...
            {anexos.length === 0 ?
              ...
              <div className="h-7 px-2 flex items-center border-r-[0.5px] border-slate-300 overflow-hidden">
                <span className="truncate font-medium text-slate-500">{anexo.attachment_name || anexo.file_name}</span>
              </div>
              <a href={anexo.file_url} target="_blank" rel="noreferrer" className="h-7 min-w-0 flex items-center gap-1.5 text-slate-500 hover:text-slate-500 px-2 border-r-[0.5px] border-slate-300 overflow-hidden">
