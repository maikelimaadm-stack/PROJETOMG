...
export default function ToggleSwitch({ checked, disabled = false, onChange, className = "", checkedClassName = "bg-white hover:bg-slate-50" }) {
  return (
    <button
      ...
      className={`!h-4 !w-8 relative inline-flex items-center transition-colors !border !border-[#082e54] shadow-none rounded-[1px] ${checked ? checkedClassName : "bg-white hover:bg-slate-50"} ${disabled ? "opacity-60 cursor-not-allowed" : "cursor-pointer"} ${className}`}
