...
const CAT_LABELS = { Planejado: "PLANEJADO", Emergencial: "EMERGENCIAL", ReposicaoEstoque: "REPOSIÇÃO ESTOQUE", Manutencao: "MANUTENÇÃO", Investimento: "INVESTIMENTO", Operacional: "OPERACIONAL", Outros: "OUTROS" };

const COLUNAS_DISPONIVEIS = [
  { id: "selecao", label: "Seleção", default: true, fixo: true, width: 25 },
  { id: "acoes", label: "Ações", default: true, fixo: true, width: 25 },
  { id: "nome", label: "Nome", default: true, sortable: true, align: "left", width: 240 },
  { id: "categoria", label: "Categoria", default: true, sortable: true, align: "left", width: 150 },
  { id: "requer_aprovacao", label: "Requer Aprovação", default: true, sortable: true, align: "center", width: 120 },
  { id: "ativo", label: "Ativo", default: true, sortable: true, align: "center", width: 80 },
  { id: "descricao", label: "Descrição", default: false, sortable: false, align: "left", width: 260 },
];

const DEFAULT_VISIBLE = COLUNAS_DISPONIVEIS.filter(c => c.default).map(c => c.id);
const STORAGE_PREFIX = "colunas_motivos_compra";
...
function getFieldValue(item, colId) {
  ...
  if (colId === "requer_aprovacao") return item.requer_aprovacao ? "Sim" : "Não";
  if (colId === "ativo") return item.ativo !== false ? "Ativo" : "Inativo";
  if (colId === "descricao") return item.descricao || "";
  return "";
}

export default function TabelaMotivosCompra({ motivos = [], onEdit, onDelete, showConfigColunas, setShowConfigColunas }) {
  ...
  const sortedMotivos = useMemo(() => {
    let filtered = motivos.filter(m => COLUNAS_DISPONIVEIS.filter(col => !col.fixo).every(col => { const f = filtrosColunas[col.id] || []; if (f.length === 0) return true; return f.includes(getFieldValue(m, col.id)); }));
    ...
    return filtered;
  }, [motivos, filtrosColunas, sortConfig]);

  useEffect(() => { setSelectedItems(prev => prev.filter(id => motivos.some(m => m.id === id))); }, [motivos]);
  ...
  const toggleSelectAll = () => { if (selectedItems.length === sortedMotivos.length && sortedMotivos.length > 0) { setSelectedItems([]); return; } setSelectedItems(sortedMotivos.map(r => r.id)); };
  ...
  const columnOptions = useMemo(() => { const o = {}; COLUNAS_DISPONIVEIS.filter(c => !c.fixo).forEach(col => { o[col.id] = [...new Set(motivos.map(item => getFieldValue(item, col.id)).filter(Boolean))].sort((a, b) => a.localeCompare(b, "pt-BR", { numeric: true, sensitivity: "base" })); }); return o; }, [motivos]);
  ...
  const renderCell = (item, colId) => {
    ...
    if (colId === "categoria") {
      const colors = { Planejado: "border-blue-300 text-blue-700", Emergencial: "border-red-300 text-red-700", ReposicaoEstoque: "border-teal-300 text-teal-700", Manutencao: "border-amber-300 text-amber-700", Investimento: "border-violet-300 text-violet-700", Operacional: "border-emerald-300 text-emerald-700", Outros: "border-slate-300 text-slate-600" };
      return <Badge variant="outline" className={`text-[10px] ${colors[item.categoria] || ""}`}>{CAT_LABELS[item.categoria] || item.categoria}</Badge>;
    }
    if (colId === "requer_aprovacao") return item.requer_aprovacao ? <Badge className="text-[10px] bg-amber-100 text-amber-700">Sim</Badge> : <span className="text-slate-400">Não</span>;
  ...
  return (
    <div className="space-y-1 overflow-hidden">
      <div className="flex justify-between items-center px-1 gap-2 flex-wrap">
        <div className="text-xs text-slate-500">{sortedMotivos.length} de {motivos.length} registros</div>
      ...
      <Card className="overflow-hidden">
        <CardContent className="p-0 overflow-hidden">
          <div ref={scrollContainerRef} className="relative w-full overflow-auto max-h-[calc(100dvh-240px)] md:max-h-[calc(100dvh-150px)]" style={{ overscrollBehavior: 'none', WebkitOverflowScrolling: 'touch' }}>
            <Table ref={tableRef} className={`w-full ${isMobile ? "min-w-[600px]" : "min-w-[800px]"} border-separate border-spacing-0 table-fixed`}>
              ...
              <TableBody>
                {sortedMotivos.length === 0 ? (
                  <TableRow><TableCell colSpan={colunasOrdenadas.length} className="text-center py-8 text-xs text-slate-400 border border-gray-300">Nenhum motivo de compra encontrado</TableCell></TableRow>
      ...
      <ConfiguracaoColunasMapaDialog open={showConfigColunas} onOpenChange={setShowConfigColunas} colunasDisponiveis={COLUNAS_DISPONIVEIS} colunasVisiveis={colunasVisiveis} colunasOrdem={colunasOrdem} toggleColuna={toggleColuna} handleDragEnd={handleDragEnd} droppableId="colunas-motivos-compra" />
    </div>
  );
}