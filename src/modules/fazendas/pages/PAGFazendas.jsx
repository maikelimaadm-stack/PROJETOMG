...
import FORMFazendas from "@/modules/fazendas/components/FORMFazendas";
import TBLFazendas from "@/modules/fazendas/components/TBLFazendas";
import { FazendasModuleDefinition } from "@/modules/fazendas/config/moduleDefinition";
import { useAuth } from "@/shared/contexts/AuthContext";
import {
  getFazendasExcelExportConfig,
  getFazendasPdfExportConfig,
  saveFazendasExcelExportConfig,
  saveFazendasPdfExportConfig,
} from "@/modules/fazendas/config/exportConfig";

const DEFAULT_RESPONSE = { items: [], total: 0, page: 1, pageSize: 50, totalPages: 1 };
const repository = FazendasModuleDefinition.repository;
const labels = {
  title: `Cadastro de ${FazendasModuleDefinition.pluralLabel}`,
  singular: FazendasModuleDefinition.singularLabel,
  plural: FazendasModuleDefinition.pluralLabel,
};

export default function PAGFazendas() {
  ...
  const { data = DEFAULT_RESPONSE } = useQuery({
    queryKey: ["fazendas", queryPage, queryPageSize, search],
  ...
  const handleExportPdf = () => {
    const config = getFazendasPdfExportConfig();
  ...
  const handleExportExcel = () => {
    const config = getFazendasExcelExportConfig();
    const columns = visibleTableData.allColumns || visibleTableData.columns || [];
    const rows = visibleTableData.allRows || visibleTableData.rows || [];
    const selectedColumns = config.useConfiguredColumns && config.columnIds.length
      ? columns.filter((column) => config.columnIds.includes(column.id))
  ...
  return (
    <div className="h-full min-h-0 overflow-hidden bg-white flex flex-col">
      ...
      <EmpConfiguracaoExportacaoDialog
        ...
        initialConfig={getFazendasPdfExportConfig()}
        tipo="pdf"
        onSaveConfig={(config) => saveFazendasPdfExportConfig(config)}
      />
      ...
      <EmpConfiguracaoExportacaoDialog
        ...
        columns={visibleTableData.allColumns || visibleTableData.columns || []}
        initialConfig={getFazendasExcelExportConfig()}
        tipo="excel"
        onSaveConfig={(config) => saveFazendasExcelExportConfig(config)}
      />
