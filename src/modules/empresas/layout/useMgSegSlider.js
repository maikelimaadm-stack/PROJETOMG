...
/**
 * Posiciona indicador deslizante sob a aba ativa (seg-tab).
 * extendX: expande a largura além da aba (px para cada lado).
 */
export default function useMgSegSlider(
  ...
  useEffect(() => {
    ...
    const { extendX = 0, bottom = 0 } = options;
    ...
    const updateSlider = () => {
      ...
      const width = Math.max(0, activeEl.offsetWidth + extendX * 2);
      const left = activeEl.offsetLeft - extendX;
