...
import FormularioCentroCusto from "../components/financeiro/FormularioCentroCusto";
import TabelaCentrosCusto from "../components/financeiro/TabelaCentrosCusto";
...
export default function CentrosCusto() {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.CentroCusto.create({ ...data, empresa_id: empresaId }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (ids) => {
      for (const id of ids) {
        const hasChildren = centros.some(c => c.centro_custo_pai_id === id);
        if (hasChildren) throw new Error("Este centro possui sub-centros vinculados. Exclua-os primeiro.");
        await base44.entities.CentroCusto.delete(id);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["centros_custo"] });
      toast.success("Centro(s) excluído(s)!");
    },
    ...
    <div className="p-1 md:p-1 space-y-1">
      {!showForm && (
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 bg-white rounded px-1 py-1 shadow-sm border-b border-slate-200">
          <div>
            <h1 className="font-bold text-slate-800">Centros de Custo</h1>
          </div>
      ...
      <ConfirmDialog
        ...
        description={deleteState.ids.length > 1 ? `Deseja realmente excluir ${deleteState.ids.length} centros de custo selecionados?` : 'Deseja realmente excluir este centro de custo?'}
        confirmText="Excluir"
        cancelText="Cancelar"
        variant="destructive"
