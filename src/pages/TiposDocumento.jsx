...
import FormularioTipoDocumento from "../components/financeiro/FormularioTipoDocumento";
import TabelaTiposDocumento from "../components/financeiro/TabelaTiposDocumento";
...
export default function TiposDocumento() {
  ...
  const { data: tipos = [] } = useQuery({
    queryKey: ["tipos_documento", empresaId],
    queryFn: async () => {
      const all = await base44.entities.TipoDocumento.list();
      return all.filter(t => t && t.empresa_id === empresaId);
    },
    enabled: !!empresaId,
  });

  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.TipoDocumento.create({ ...data, empresa_id: empresaId }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["tipos_documento"] }); setShowForm(false); setEditingItem(null); toast.success("Tipo de documento criado!"); },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["tipos_documento"] }); setShowForm(false); setEditingItem(null); toast.success("Tipo de documento atualizado!"); },
    onError: (err) => toast.error("Erro: " + (err.message || "Erro desconhecido")),
  });

  const deleteMutation = useMutation({
    mutationFn: async (ids) => { for (const id of ids) await base44.entities.TipoDocumento.delete(id); },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["tipos_documento"] }); toast.success("Tipo(s) excluído(s)!"); },
    ...
    <div className="p-1 md:p-1 space-y-1">
      {!showForm && (
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 bg-white rounded px-1 py-1 shadow-sm border-b border-slate-200">
          <div><h1 className="font-bold text-slate-800">Tipos de Documento</h1></div>
      ...
      <AnimatePresence mode="wait">
        {showForm ? (
          <FormularioTipoDocumento key="form" initialData={editingItem} onSubmit={handleSubmit} onCancel={() => { setShowForm(false); setEditingItem(null); }} tiposExistentes={tipos} />
        ) : (
          <TabelaTiposDocumento key="table" tipos={tipos} onEdit={handleEdit} onDelete={handleRequestDelete} showConfigColunas={showConfigColunas} setShowConfigColunas={setShowConfigColunas} />
        )}
      </AnimatePresence>
      <ConfirmDialog open={deleteState.open} onOpenChange={(open) => setDeleteState(prev => ({ ...prev, open }))} title="Confirmar exclusão" description={deleteState.ids.length > 1 ? `Deseja excluir ${deleteState.ids.length} tipos selecionados?` : "Deseja excluir este tipo de documento?"} confirmText="Excluir" cancelText="Cancelar" variant="destructive" onConfirm={handleConfirmDelete} />
    </div>
  );
}