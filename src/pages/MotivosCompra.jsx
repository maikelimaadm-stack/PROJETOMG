...
import FormularioMotivoCompra from "../components/financeiro/FormularioMotivoCompra";
import TabelaMotivosCompra from "../components/financeiro/TabelaMotivosCompra";

export default function MotivosCompra() {
  ...
  const { data: motivos = [] } = useQuery({
    queryKey: ["motivos_compra", empresaId],
    queryFn: async () => {
      const all = await base44.entities.MotivoCompra.list();
      return all.filter(m => m && m.empresa_id === empresaId);
    },
    enabled: !!empresaId,
  });

  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.MotivoCompra.create({ ...data, empresa_id: empresaId }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["motivos_compra"] }); setShowForm(false); setEditingItem(null); toast.success("Motivo de compra criado!"); },
    onError: (err) => toast.error("Erro: " + (err.message || "Erro desconhecido")),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.MotivoCompra.update(id, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["motivos_compra"] }); setShowForm(false); setEditingItem(null); toast.success("Motivo de compra atualizado!"); },
    onError: (err) => toast.error("Erro: " + (err.message || "Erro desconhecido")),
  });

  const deleteMutation = useMutation({
    mutationFn: async (ids) => { for (const id of ids) await base44.entities.MotivoCompra.delete(id); },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["motivos_compra"] }); toast.success("Motivo(s) excluído(s)!"); },
    ...
    <div className="p-1 md:p-1 space-y-1">
      {!showForm && (
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 bg-white rounded px-1 py-1 shadow-sm border-b border-slate-200">
          <div><h1 className="font-bold text-slate-800">Motivos de Compra</h1></div>
      ...
      <AnimatePresence mode="wait">
        {showForm ? (
          <FormularioMotivoCompra key="form" initialData={editingItem} onSubmit={handleSubmit} onCancel={() => { setShowForm(false); setEditingItem(null); }} motivosExistentes={motivos} />
        ) : (
          <TabelaMotivosCompra key="table" motivos={motivos} onEdit={handleEdit} onDelete={handleRequestDelete} showConfigColunas={showConfigColunas} setShowConfigColunas={setShowConfigColunas} />
        )}
      </AnimatePresence>
      <ConfirmDialog open={deleteState.open} onOpenChange={(open) => setDeleteState(prev => ({ ...prev, open }))} title="Confirmar exclusão" description={deleteState.ids.length > 1 ? `Deseja excluir ${deleteState.ids.length} motivos selecionados?` : "Deseja excluir este motivo de compra?"} confirmText="Excluir" cancelText="Cancelar" variant="destructive" onConfirm={handleConfirmDelete} />
    </div>
  );
}