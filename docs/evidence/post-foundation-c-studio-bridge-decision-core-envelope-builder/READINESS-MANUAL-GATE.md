...
readiness = `studio_bridge_decision_core_envelope_builder_ready_for_enterprise_audit`. `readyForEnterpriseBuilderAudit:true`; `readyForConsumerRuntimeImplementation:false`. Manual gate: `authorizesBuilderImplementation:true`; `authorizesConsumerRuntime:false`; `authorizesCoreEnvelopeAmendment:false`.
