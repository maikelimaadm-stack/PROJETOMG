...
Implementa `createBridgeDecisionCoreEnvelopeBuilder(config) -> { build(bridgeDecision) }`. O `build` roda o pipeline real de 23 estágios em ordem, para no primeiro blocker (atômico — nenhum envelope em falha), recomputa e compara o digest exatamente, impõe same-decision atomicity, e constrói um Core Envelope v2 imutável.
