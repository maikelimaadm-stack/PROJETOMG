...
The same source handoff must yield the same bridge decision, the same target descriptor and the same digest, with no replay side effects. Idempotent by contract; bridge execution is a future slice (`bridgeExecutionImplemented` = false).
