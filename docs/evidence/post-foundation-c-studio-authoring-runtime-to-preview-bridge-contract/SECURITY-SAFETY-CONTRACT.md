...
Every real-effect allowance (network/storage/filesystem/backend/prisma/real-data/product-exposure/preview-mount/app-touch/route/menu/module-generation/certification) is **false**; `anyRealAllowed` = false. The contract is reversible by non-consumption and remains headless/metadata-only.
