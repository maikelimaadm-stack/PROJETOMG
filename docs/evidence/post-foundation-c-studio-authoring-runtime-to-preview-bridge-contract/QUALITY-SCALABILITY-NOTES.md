...
The contract is pure data with deterministic FNV-1a digests, enabling replay-stable equality and cheap tamper detection. The part decomposition (one descriptor per concern) keeps each concern independently verifiable and lets a future implementation plan and implementation slice consume the contract without re-deriving its rules. Determinism (no clock/randomness) makes the contract diffable and cacheable.
