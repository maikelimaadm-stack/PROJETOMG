...
The pre-existing legacy Studio prototype exposure debt is **out of scope** for this slice and is **not touched**. This slice adds only the isolated headless bridge-implementation-plan subtree, its test, its gate, its evidence docs and the governance-registry + package.json wiring. No prototype file is relinked, imported, moved or re-exposed.
