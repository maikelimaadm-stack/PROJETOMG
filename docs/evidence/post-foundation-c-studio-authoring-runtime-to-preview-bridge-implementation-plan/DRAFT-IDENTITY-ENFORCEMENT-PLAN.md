...
Plans strict, explicit draft-identity enforcement: explicit draft id required, single-draft fallback forbidden, missing/unknown/ambiguous ids fail closed. The upstream runtime is **not** altered by this plan (`runtimeAlteredByThisPlan` = false). `strictDraftIdentityEnforcementImplemented` = false.
