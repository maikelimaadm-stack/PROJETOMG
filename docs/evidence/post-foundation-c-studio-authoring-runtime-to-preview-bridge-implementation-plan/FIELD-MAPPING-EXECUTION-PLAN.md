...
Plans (does not execute) the 11 source→target mappings. Each mapping is `planned`, `implemented` = false. Unknown transforms fail closed; critical defaults and lossy critical mappings forbidden; silent critical rename forbidden; deterministic order; every target critical field covered. `anyUnknownTransform`/`anyCriticalDefault`/`anyLossyCritical`/`anyImplemented` = false; `everyCriticalMapped` = true.
