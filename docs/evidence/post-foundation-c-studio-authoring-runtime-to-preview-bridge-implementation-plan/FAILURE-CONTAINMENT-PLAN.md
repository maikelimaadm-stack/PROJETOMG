...
Plans that a failed bridge run leaves no partial target descriptor or bridge decision, mutates neither source nor target, and rolls back purely by non-consumption (no external cleanup / database rollback / filesystem cleanup). `failureContainmentImplemented` = false.
