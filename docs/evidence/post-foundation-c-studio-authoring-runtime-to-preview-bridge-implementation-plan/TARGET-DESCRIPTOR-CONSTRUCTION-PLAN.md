# Target Descriptor Construction Plan
...
Plans (does not build) the synthetic `module_preview_sandbox_candidate` target descriptor. Synthetic-only, metadata-only: no real payload, no mount, no route/menu, no product exposure, no module generation, no persistence. `targetDescriptorBuilderImplemented` = false.
