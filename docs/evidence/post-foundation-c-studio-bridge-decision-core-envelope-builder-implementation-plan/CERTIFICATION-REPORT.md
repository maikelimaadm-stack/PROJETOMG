...
- verification.ok = `true`, blockers = [].
- planOnly = `true`; phases = 22; risks = 17.
- owner = `consumer_runtime`; architecture = `ARCHITECTURE_1`.
- B-CORE-ENVELOPE-BUILDER closed = `true`; resolved by plan = `true`.
- readyForEnterprisePlanAudit = `true`; readyForBuilderImplementation = `false`.
- manifest overall = `fnv1a-69ad97e9` (23 partes).
